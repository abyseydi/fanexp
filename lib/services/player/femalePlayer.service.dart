...
class FemalePlayerService {
  ...
  Future<List<FemalePlayerEntity>> getFemalePlayers() async {
    try {
      final token = await _prefs.getUserKeyValue('token');
      final headers = headersAuth(token);
      final uri = Uri.parse('$apiUrl/v1/joueuses');
      ...
      if (resp.statusCode == 200) {
        ...
        if (decoded is List) {
          return decoded
              .map(
                (e) => FemalePlayerEntity.fromJson(e as Map<String, dynamic>),
              )
              .toList();
        } else {
      ...
      throw Exception('Erreur getFemalePlayers(): $e');
    }
  }
}
